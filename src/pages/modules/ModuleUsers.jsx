...
export default function ModuleUsers() {
  ...
  const filteredUsers = users.filter(u => {
    ...
    return fullName.includes(term) || (u.email || '').includes(term) || (u.document_number || '').includes(term);
  });
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem 2rem', background: 'rgba(248, 244, 238, 0.5)', borderBottom: '1px solid rgba(45, 55, 63, 0.05)' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input 
            ...
            style={{ padding: '0.5rem 1rem 0.5rem 2rem', borderRadius: '8px', border: '1px solid rgba(45, 55, 63, 0.1)', outline: 'none', width: '200px', fontSize: '0.85rem' }} 
          />
        </div>
                ...
                {filteredUsers.map(u => (
                  <tr key={u.id} style={{ opacity: activeTab === 'deleted' ? 0.6 : 1, transition: 'background 0.2s', borderBottom: '1px solid rgba(0,0,0,0.03)' }}>
                    ...
                    <td style={{ textAlign: 'right' }}>
                      {activeTab === 'deleted' ? (
                        <button onClick={() => handleRestore(u.id)} style={{ background: 'rgba(105, 151, 126, 0.1)', border: 'none', color: 'var(--color-tertiary)', cursor: 'pointer', padding: '0.5rem', borderRadius: '6px', display: 'inline-flex', alignItems: 'center', gap: '0.3rem', fontWeight: '600' }} title="Restaurar Usuario">
                          <RefreshCcw size={14} /> Restaurar
                        </button>
                      ) : (
                        <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'flex-end' }}>
                          <button onClick={() => openEditModal(u)} style={{ background: 'transparent', border: '1px solid rgba(0,0,0,0.1)', color: 'var(--color-text-muted)', cursor: 'pointer', padding: '0.4rem', borderRadius: '6px' }} title="Editar">
                            <Edit2 size={16} />
                          </button>
                          <button onClick={() => handleDelete(u.id)} style={{ background: 'rgba(211, 73, 55, 0.05)', border: 'none', color: 'var(--color-primary)', cursor: 'pointer', padding: '0.4rem', borderRadius: '6px' }} title="Mover a Papelera">
                            <Trash2 size={16} />
                          </button>
                        </div>
                      )}
        ...
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(45, 55, 63, 0.5)', backdropFilter: 'blur(4px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', padding: '2rem', borderRadius: '16px', width: '90%', maxWidth: '600px', boxShadow: '0 20px 50px rgba(0,0,0,0.2)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.4rem' }}>{editingUser ? 'Editar Registro' : 'Nuevo Registro'}</h3>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
              ...
              <div>
                ...
                <input name="first_name" type="text" required value={formData.first_name} onChange={handleFormChange} style={{ width: '100%', padding: '0.7rem', borderRadius: '8px', border: '1px solid rgba(45, 55, 63, 0.2)', outline: 'none' }} />
              </div>
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.3rem', fontWeight: 600 }}>Apellidos</label>
                <input name="last_name" type="text" required value={formData.last_name} onChange={handleFormChange} style={{ width: '100%', padding: '0.7rem', borderRadius: '8px', border: '1px solid rgba(45, 55, 63, 0.2)', outline: 'none' }} />
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.3rem', fontWeight: 600 }}>Doc Identidad</label>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <select name="document_type" value={formData.document_type} onChange={handleFormChange} style={{ padding: '0.7rem', borderRadius: '8px', border: '1px solid rgba(45, 55, 63, 0.2)', outline: 'none', width: '35%' }}>
                    <option value="DNI">DNI</option>
                    <option value="CE">CE</option>
                    <option value="PASSPORT">Pasaporte</option>
                  </select>
                  <input name="document_number" type="text" required value={formData.document_number} onChange={handleFormChange} style={{ width: '65%', padding: '0.7rem', borderRadius: '8px', border: '1px solid rgba(45, 55, 63, 0.2)', outline: 'none' }} />
                </div>
              </div>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '0.85rem', marginBottom: '0.3rem', fontWeight: 600 }}>Rol del Sistema</label>
                <select name="role" value={formData.role} onChange={handleFormChange} style={{ width: '100%', padding: '0.7rem', borderRadius: '8px', border: '1px solid rgba(45, 55, 63, 0.2)', outline: 'none' }}>
              ...
              <div style={{ gridColumn: '1 / -1' }}>
                ...
                <input name="password" type="password" required={!editingUser} value={formData.password} onChange={handleFormChange} style={{ width: '100%', padding: '0.7rem', borderRadius: '8px', border: '1px solid rgba(45, 55, 63, 0.2)', outline: 'none' }} />
              </div>
